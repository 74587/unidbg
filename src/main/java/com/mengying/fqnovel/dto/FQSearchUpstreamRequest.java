 ...
 * 承载内部搜索链路所需的完整参数集合。
 */
public class FQSearchUpstreamRequest {
    ...
    public static FQSearchUpstreamRequest from(FQSearchRequest request) {
        FQSearchUpstreamRequest upstreamRequest = new FQSearchUpstreamRequest();
        if (request == null) {
            return upstreamRequest;
        }
        upstreamRequest.setQuery(request.getQuery());
        upstreamRequest.setOffset(request.getOffset());
        upstreamRequest.setCount(request.getCount());
        upstreamRequest.setPassback(request.getPassback());
        upstreamRequest.setTabType(request.getTabType());
        upstreamRequest.setSearchId(request.getSearchId());
        return upstreamRequest;
    }

    public FQSearchUpstreamRequest copy() {
        FQSearchUpstreamRequest copied = new FQSearchUpstreamRequest();
        copied.setQuery(query);
        copied.setOffset(offset);
        copied.setCount(count);
        copied.setPassback(passback);
        copied.setTabType(tabType);
        copied.setSearchId(searchId);
        copied.setBookshelfSearchPlan(bookshelfSearchPlan);
        copied.setFromRs(fromRs);
        copied.setUserIsLogin(userIsLogin);
        copied.setBookstoreTab(bookstoreTab);
        copied.setSearchSource(searchSource);
        copied.setClickedContent(clickedContent);
        copied.setSearchSourceId(searchSourceId);
        copied.setUseLynx(useLynx);
        copied.setUseCorrect(useCorrect);
        copied.setTabName(tabName);
        copied.setIsFirstEnterSearch(isFirstEnterSearch);
        copied.setClientAbInfo(clientAbInfo);
        copied.setLastSearchPageInterval(lastSearchPageInterval);
        copied.setLineWordsNum(lineWordsNum);
        copied.setLastConsumeInterval(lastConsumeInterval);
        copied.setPadColumnCover(padColumnCover);
        copied.setKlinkEgdi(klinkEgdi);
        copied.setNormalSessionId(normalSessionId);
        copied.setColdStartSessionId(coldStartSessionId);
        copied.setCharging(charging);
        copied.setScreenBrightness(screenBrightness);
        copied.setBatteryPct(batteryPct);
        copied.setDownSpeed(downSpeed);
        copied.setSysDarkMode(sysDarkMode);
        copied.setAppDarkMode(appDarkMode);
        copied.setFontScale(fontScale);
        copied.setIsAndroidPadScreen(isAndroidPadScreen);
        copied.setNetworkType(networkType);
        copied.setRomVersion(romVersion);
        copied.setCurrentVolume(currentVolume);
        copied.setCdid(cdid);
        copied.setNeedPersonalRecommend(needPersonalRecommend);
        copied.setPlayerSoLoad(playerSoLoad);
        copied.setGender(gender);
        copied.setComplianceStatus(complianceStatus);
        copied.setHarStatus(harStatus);
        return copied;
    }
